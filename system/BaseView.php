...
<?php
	...
	private function parse_report_html()
	{
		...
		$dom->querySelector('.record-title')->classList->add('oculto');
		$xpath = new DOMXPath($dom);
		$page_body = $dom->getElementById("page-report-body");
		...
		$campoTitulo= $xpath->query('//h4[@class="record-title"]');
		foreach($campoTitulo as $titulo){
			
			$titulo->setAttribute('class','oculto');

		}
		$xpath = new DOMXPath($dom);

		$listaBotones=  $xpath->query('//div[@class="row justify-content-center"]');

		foreach($listaBotones as $contenedor){
			
			$contenedor->setAttribute('class','oculto');

		}
		$xpath = new DOMXPath($dom);

		$listaBotones=  $xpath->query('//div[@class="bg-light p-3 mb-3"]');
	...
	{
		$html = html_entity_decode(fixWrongUTF8Encoding($html));
