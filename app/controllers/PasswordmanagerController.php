...
class PasswordmanagerController extends BaseController{
	...
	function postresetlink(){
		if(!empty($this->post->email)){
			...
			$user = $db->getOne($tablename, array('idUSUARIO', 'USUARIO'));
			if(!empty($user)){
				...
				if($mailer->send_mail($email, $mailtitle, $mailbody) == true){
					...
					$msg = "Error al enviar un correo electrónico Por favor, póngase en contacto con el administrador del sistema para más información";
					$this->render_view("errors/error_general.php", $msg, "info_layout.php");
				}
			}
			else{
				$this->set_page_error("La dirección de correo electrónico no está registrada en el sistema");
				$this->render_view("passwordmanager/index.php", null, "info_layout.php");
			}
		}
		else{
				...
				if($password_has_not_expired){
					if(!empty($_POST['password'])){
						...
						if($password == $cpassword){
							...
							$this->set_page_error("La confirmación de su contraseña no es consistente");
