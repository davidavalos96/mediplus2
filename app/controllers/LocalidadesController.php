<?php 
/**
 * Localidades Page Controller
 * @category  Controller
 */
class LocalidadesController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				localidades.date_deleted LIKE ? OR 
				localidades.is_deleted LIKE ?
			)";
			$search_params = array(
			...
			$this->view->search_template = "localidades/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("localidades.id", ORDER_TYPE);
		}
		if($fieldname){
		...
		$page_title = $this->view->page_title = "Localidades";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		$this->render_view("localidades/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("localidades.id", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("localidades/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("provincia","localidad");
		...
		$this->render_view("localidades/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","provincia","localidad");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'provincia' => 'required|numeric',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("localidades.id", $rec_id);;
		...
		return $this->render_view("localidades/edit.php", $data);
	}
	/**
     * Update single field
	 * @param $rec_id (select record by table primary key)
	...
	function delete($rec_id = null){
		...
		$db->where("localidades.id", $arr_rec_id, "in");
		$modeldata = array(
			"is_deleted" => "1",
