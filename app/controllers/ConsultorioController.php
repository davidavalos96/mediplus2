<?php 
/**
 * Consultorio Page Controller
 * @category  Controller
 */
class ConsultorioController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$this->view->search_template = "consultorio/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("consultorio.CODIGO", ORDER_TYPE);
		}
		if($fieldname){
		...
		$page_title = $this->view->page_title = "Consultorio";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		$this->render_view("consultorio/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("consultorio.CODIGO", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("consultorio/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("CODIGO","DESCRIPCION");
			...
			if($this->validated()){
				...
				if($rec_id){
					...
					return	$this->redirect("consultorio");
				}
				else{
					$this->set_page_error();
				}
		...
		$this->render_view("consultorio/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("consultorio.CODIGO", $rec_id);;
		...
		return $this->render_view("consultorio/edit.php", $data);
	}
	/**
     * Update single field
	 * @param $rec_id (select record by table primary key)
	...
	function delete($rec_id = null){
		...
		$db->where("consultorio.CODIGO", $arr_rec_id, "in");
		$modeldata = array(
			"is_deleted" => "1",
